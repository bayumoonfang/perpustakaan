...
const BarcodeButton = (props) => {
    ...
    return (
        <div>
        <button class="btn btn-sm btn-outline-success ml-2" onClick={()=>setShowModal(true)}>Barcode</button>
        <Modal
        size="md"
		backdrop="static"
        keyboard={false}
        ...
        >
            ...
            <div class="row mt-2 mb-2">
                <div class="col-md-auto">
                    <button disabled={loadingGenerate} className='btn btn-outline-success' onClick={onRegenerate}>{loadingGenerate ? 'Loading...':'Regenerate Barcode'}</button>
                </div>
                <div class="col-md-auto">
                    <p className='text-warning'><em>
                        **Regenerate Barcode hanya akan menambah jumlah barcode sesuai Qty buku (jika kurang) dan tidak akan menghapus atau mengurangi barcode yang telah dibuat sebelumnya. 
                        **Perubahan Kode buku master tidak akan merubah barcode yang telah digenerate sebelumnya.
                    </em></p>
                </div>
            </div>
            <hr/>
            {listBarcode && listBarcode.length>0 &&
                <div class="row mt-2 mb-2">
                    <div class="col-md-auto"><button onClick={selectAllBarcode} className='btn btn-sm btn-outline-info mb-2'>{isAll ? 'Batal Pilih Semua':'Pilih Semua'}</button></div>
                    {canCetak && <div class="col-md-auto"><button onClick={cetakBarcode} className='btn btn-sm btn-outline-primary mb-2'>Cetak</button></div>}
                </div>
                }
            ...
            <table className='table'>
                ...
                <tbody>
                    {!loadingData && listBarcode.map((item,index)=>{
                        return(
                            <tr>
                                <td>
                                    <div class="form-check">
                                    <input class="form-check-input position-static" type="checkbox" id="blankCheckbox" checked={item?.selected} onClick={()=>selectBarcodeItem(item)} value="option1" aria-label="..."/>
                                    </div>
                                </td>
                                <td>{item?.barcode}</td>
                            </tr>
            ...
            <Modal.Footer>
                <button disabled={loadingGenerate} className='btn btn-outline-secondary' onClick={()=>setShowModal(false)}>Close</button>
            </Modal.Footer>
        </Modal>
        </div>
    );
