				...
				<div class="page-title-box d-flex align-items-center justify-content-between">
					...
					<div class="page-title-right">
						<ol class="breadcrumb m-0">
							...
							<li class="breadcrumb-item"><a href="{{admin_url('kategori-buku')}}">Kategori Buku</a></li>
		...
		<div class="row">
			<div class="col-6">
				<div class="card">
					<div class="card-body">
						{{show_status()}}
						<form action="{{ $edit ? admin_url("kategori-buku/$kategori_buku->id/update"):admin_url('kategori-buku/add')}}" method="post">
							{{csrf_token()}}
							<div class="row">
								<div class="col-md-12">
									<div class="form-group">
										...
										<select name='library' required class="select_perpustakaan form-control {{get_error('library') ? 'is-invalid':''}}">
											...
											@foreach ($library as $item)
												<option value="{{$item->id}}"  {{$edit && $item->id==$kategori_buku->library ? 'selected':''}}>{{ucfirst($item->library)}}</option>
											@endforeach
										</select>
									...
									<div class="form-group">
										...
										<input name="category" type="text" class="form-control {{get_error('category') ? 'is-invalid':''}}" id="category" placeholder="Kategori Buku" required value="{{$edit ? $kategori_buku->category : old('category')}}" autocomplete="off">
									...
									<div class="form-group">
										...
										<select name='status' required class="form-control {{get_error('status') ? 'is-invalid':''}}">
											<option value="1" {{$edit && $kategori_buku->status=='1' ?'selected' :''}} >Aktif</option>
											<option value="0" {{$edit && $kategori_buku->status=='0' ?'selected' :''}}>Tidak Aktif</option>
										</select>
										@if (get_error('status'))
        ...
        $.confirm({
            ...
            buttons: {
                ...
                'Ya': {
                    btnClass: 'btn-danger',
                    action: function(){
                        window.location.href = "{{admin_url('kategori-buku')}}/"+data_id+'/delete';
                    }
                },
            },
