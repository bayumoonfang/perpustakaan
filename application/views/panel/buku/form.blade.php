...
@section('content')
    <div class="page-content">
        <div class="container-fluid">
            ...
            <form action="{{ $edit ? admin_url("buku/$buku->id/update") : admin_url('buku/add') }}" method="post"
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <select name='library' id='select_perpustakaan' required
                                                class="select_perpustakaan select2-container form-control {{ get_error('library') ? 'is-invalid' : '' }}">
                                                <option value="">Pilih Perpustakaan</option>
                                                @foreach ($library as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ !is_admin() && $item->school != current_user('user_sekolahid') ? 'disabled' : '' }}
                                                        {{ ($edit && $item->id == $buku->library ? 'selected' : old('library')) ? 'selected' : '' }}>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name='rak' id='select_rak' required
                                                class="select_perpustakaan select2-container form-control {{ get_error('rak') ? 'is-invalid' : '' }}">
                                                <option value="" selected disabled>Pilih Rak</option>
                                                @foreach ($rak as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ ($edit && $item->id == $buku->rak ? 'selected' : old('rak')) ? 'selected' : '' }}>
                                                        {{ ucfirst($item->rack) }}</option>
                                                @endforeach
                                            </select>
                                            @if (get_error('rak'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name='category' required
                                                class="select_perpustakaan form-control {{ get_error('category') ? 'is-invalid' : '' }}">
                                                <option value="" selected disabled>Pilih Kategori Buku</option>
                                                @foreach ($kategori_buku as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ ($edit && $item->id == $buku->category ? 'selected' : old('rak')) ? 'selected' : '' }}>
                                                        {{ ucfirst($item->category) }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group">
                                            <label for="status">Klasifikasi Subjek</label>
                                            <select name='subject' required
                                                class="select_subject form-control {{ get_error('subject') ? 'is-invalid' : '' }}">
                                                <option value="" selected disabled>Pilih Klasifikasi Subjek</option>
                                                @foreach ($subjek_buku as $item)
                                                    <option value="{{ $item->id }}"
                                                        data-minvalue ="{{ $item->min_value }}"
                                                        data-maxvalue ="{{ $item->max_value }}"
                                                        data-lengthvalue ="{{ $item->length_value }}"
                                                        {{ ($edit && $item->id == $buku->books_subjectid ? 'selected' : old('rak')) ? 'selected' : '' }}>
                                                        {{ $item->min_value }} - {{ ucfirst($item->name) }}</option>
                                                @endforeach
                                            </select>
                                            @if (get_error('subject'))
                                                <div class="invalid-feedback">{{ get_error('subject') }}</div>
                                            @endif
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input name="code" type="text"
                                                class="form-control {{ get_error('code') ? 'is-invalid' : '' }}"
                                                id="code" placeholder="Kode Buku" required
                                                value="{{ $edit ? $buku->code : old('code') }}" autocomplete="off">
                                            @if (get_error('code'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input name="title" type="text"
                                                class="form-control {{ get_error('title') ? 'is-invalid' : '' }}"
                                                id="title" placeholder="Judul Buku" required
                                                value="{{ $edit ? $buku->title : old('title') }}" autocomplete="off">
                                            @if (get_error('title'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input name="bentuk" type="text"
                                                class="form-control {{ get_error('bentuk') ? 'is-invalid' : '' }}"
                                                id="bentuk" placeholder="Bentuk" required
                                                value="{{ $edit ? $buku->bentuk : old('bentuk') }}" autocomplete="off">
                                            @if (get_error('bentuk'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input name="author" type="text"
                                                class="form-control {{ get_error('author') ? 'is-invalid' : '' }}"
                                                id="author" placeholder="Pengarang" required
                                                value="{{ $edit ? $buku->author : old('author') }}" autocomplete="off">
                                            @if (get_error('author'))
                                            ...
                                            <div class="col-md-8">
                                                <div class="form-group">
                                                    ...
                                                    <input name="publisher" type="text"
                                                        class="form-control {{ get_error('publisher') ? 'is-invalid' : '' }}"
                                                        id="publisher" placeholder="Penerbit" required
                                                        value="{{ $edit ? $buku->publisher : old('publisher') }}"
                                                        autocomplete="off">
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    ...
                                                    <input name="year" type="number" min="0"
                                                        class="form-control {{ get_error('year') ? 'is-invalid' : '' }}"
                                                        id="year" placeholder="Tahun" required
                                                        value="{{ $edit ? $buku->year : old('year') }}"
                                                        autocomplete="off">
                                                    @if (get_error('year'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input name="kolasi" type="text"
                                                class="form-control {{ get_error('kolasi') ? 'is-invalid' : '' }}"
                                                id="kolasi" placeholder="Kolasi" required
                                                value="{{ $edit ? $buku->kolasi : old('kolasi') }}" autocomplete="off">
                                            @if (get_error('kolasi'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input name="isbn" type="text"
                                                class="form-control {{ get_error('isbn') ? 'is-invalid' : '' }}"
                                                id="isbn" placeholder="ISBN" required
                                                value="{{ $edit ? $buku->isbn : old('isbn') }}" autocomplete="off">
                                            @if (get_error('isbn'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input name="price" type="text"
                                                class="form-control {{ get_error('price') ? 'is-invalid' : '' }}"
                                                id="price" placeholder="price" required
                                                value="{{ $edit ? $buku->price : old('price') }}" autocomplete="off">
                                            @if (get_error('price'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name='language' required
                                                class="select_perpustakaan form-control {{ get_error('language') ? 'is-invalid' : '' }}">
                                                <option value="" selected disabled>Pilih Bahasa</option>
                                                @foreach ($bahasa as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ ($edit && $item->id == $buku->language ? 'selected' : old('language')) ? 'selected' : '' }}>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name='class' required
                                                class="select_perpustakaan form-control {{ get_error('class') ? 'is-invalid' : '' }}">
                                                <option value="" selected disabled>Pilih Kelas</option>
                                                @if ($edit)
                                                    <option value="0"
                                                        {{ ($edit && $buku->class == '0' ? 'selected' : old('class')) ? 'selected' : '' }}>
                                                        Umum</option>
                                                    @foreach ($kelas as $item)
                                                        <option value="{{ $item->kelas_id }}"
                                                            {{ ($edit && $item->kelas_id == $buku->class ? 'selected' : old('class')) ? 'selected' : '' }}>
                                                            Kelas {{ ucfirst($item->kelas_nama) }}
                                                            {{ $item->sekolah_nama }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                            @if (get_error('class'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name='mapel' required
                                                class="select_perpustakaan form-control {{ get_error('mapel') ? 'is-invalid' : '' }}">
                                                <option value="" selected disabled>Pilih Mata Pelajaran</option>
                                                @if ($edit)
                                                    <option value="0"
                                                        {{ ($edit && '0' == $buku->mapel ? 'selected' : old('mapel')) ? 'selected' : '' }}>
                                                        Umum</option>
                                                    @foreach ($mapel as $item)
                                                        <option value="{{ $item->mapel_id }}"
                                                            {{ ($edit && $item->mapel_id == $buku->mapel ? 'selected' : old('mapel')) ? 'selected' : '' }}>
                                                            {{ ucfirst($item->mapel_nama) }}
                                                            {{ is_admin() ? $item->sekolah_nama : null }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                            @if (get_error('mapel'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name='status' required
                                                ...
                                                <option value="1"
                                                    {{ $edit && $buku->status == '1' ? 'selected' : '' }}>
                                                    Aktif</option>
                                                <option value="0"
                                                    {{ $edit && $buku->status == '0' ? 'selected' : '' }}>
                                                    Tidak Aktif</option>
                                            </select>
                                            @if (get_error('status'))
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <input type="file" name="cover" id="cover"
                                        class="form-control {{ get_error('cover') ? 'is-invalid' : '' }}"
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <input type="file" name="ebook" id="ebook"
                                        class="form-control {{ get_error('ebook') ? 'is-invalid' : '' }}"
                                        accept="application/pdf,application/force-download,application/x-download,binary/octet-stream'">
                                    @if (get_error('ebook'))
                                        <div class="invalid-feedback">{{ get_error('ebook') }}</div>
                                    @endif
                                </div>
                                @if ($edit && !empty($buku->fileurl) && ($item->is_digital_book = '1'))
                                    <a class="btn btn-sm btn-success" href="{{ $buku->fileurl }}" target="_blank">View
                                        Ebook</a>
                                @endif
                            </div>
                        </div>
            ...
            $('#code').on('keyup focusout', function() {
                ...
                if ($(this).val() > $(this).data('maxvalue') || $(this).val() < $(this).data(
                    ...
                    Swal.fire({
                        icon: 'warning',
                        title: 'Warning',
                        text: 'Tidak boleh melampaui batas klasifikasi subjek',
                        // timer: 1000
                    });
                    $(this).val('');
                }
        ...
        function change_libraries(libraryId = '') {
            ...
            requestGetJSON('{{ admin_url('rak-perpustakaan/ajax/data') }}' + '/' + libraryId).done(function(response) {
                ...
                response.forEach(element => {
                    $('select[name="rak"]').append(
                        `<option value="${element.id}">${element.rack}</option>`);
                });
            })
            ...
            requestGetJSON('{{ admin_url('kelas/ajax/data') }}' + '/' + libraryId).done(function(response) {
                ...
                response.forEach(element => {
                    $('select[name="class"]').append(
                        `<option value="${element.kelas_id}">${element.kelas_nama} ${element.sekolah_nama}</option>`
                    );
                });
            })
            requestGetJSON('{{ admin_url('mapel/ajax/data') }}' + '/' + libraryId).done(function(response) {
                $('select[name="mapel"]').find('option').remove();
                $('select[name="mapel"]').append(
                    '<option value="" selected disabled>Pilih Mata Pelajaran</option>');
                $('select[name="mapel"]').append('<option value="0">Umum</option>');
                response.forEach(element => {
                    $('select[name="mapel"]').append(
                        `<option value="${element.mapel_id}">${element.mapel_nama} ${element.sekolah_nama}</option>`
                    );
                });
            })
        }
